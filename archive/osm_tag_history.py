...
OUTPUT 
- Plots with comparison of usage of particular OSM tags throughout years for features that have been described by different tags (by area, count, length).
...
for lulc, template, year in lulc_year_combinations:
    ...
    try:
        ...
        if 'esa' in template_name.lower():
            bbox_name = f"UK_UKCEH_LCM" # or "UK, ESA"
        else:
            bbox_name = f"Catalonia_MUCSC" # or "Catalonia"
...
# info - other options (might be useful in future)
"""
...
# Redundant block - another pair of deprecated tags can't be analysed through ohsome api (does not support pattern matching)
"""
    {
...
# previous block - to export to CSV
"""
